...
#Contract id is the new token we are interacting with
...
if int(al_value) <= int(ap_quan):
    approve = tokenContract.functions.approve(panRouterContractAddress, 115792089237316195423570985008687907853269984665640564039457584007913129639935).buildTransaction({
            'from': sender_address,
            'chainId': 56,
            'gas': 100000,
...
userchoice = input("What do you want to do??? BUY = b ---- SELL = s \n")

if(userchoice == 's'):
    #Enter amount of token to sell
    tokenValue = web3.toWei(input("Enter amount of " + tokensymbol + " you want to sell: "), 'ether')

    #Get Token Balance
    readable = 0
    while (readable < 1):
        balance = tokenContract.functions.balanceOf(sender_address).call()
        readable = web3.fromWei(balance,'ether')
        print("Balance: " + str(readable) + " " + tokensymbol)
    
    
    tokenValue2 = web3.fromWei(tokenValue, 'ether')
    print(f"Swapping {tokenValue2} {tokensymbol} for BNB")

    #Swaping exact Token for ETH 
    amounts = contract.functions.getAmountsOut(tokenValue, [contract_id, spend]).call()
    ...
    uservalue = input("Enter amount of BNB/BUSD you want to buy: ")
    while (decimal.Decimal(uservalue) > balReadable):
        print("Insufficient balance")
        uservalue = input("Enter VALID amount of BNB you want to buy: ")
    tokenValue = web3.toWei(uservalue, 'ether')
    tokenValue2 = web3.fromWei(tokenValue, 'ether')

    
    time.sleep(3)
    print(f"Swapping {tokenValue2} BNB/BUSD for {tokensymbol} ")
    #Swaping exact Token for ETH 
    amounts = contract.functions.getAmountsOut(tokenValue, [spend, contract_id]).call()
    ...
    signed_txn = web3.eth.account.sign_transaction(pancakeswap2_txn, private_key=config.private)
    tx_token = web3.eth.send_raw_transaction(signed_txn.rawTransaction)
    print(f"Bought {tokensymbol}: " + web3.toHex(tx_token))