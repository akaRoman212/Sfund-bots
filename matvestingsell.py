...
slippage= config.high_slippage

matic = "https://speedy-nodes-nyc.moralis.io/b6905c301ad6187a708272d7/polygon/mainnet"
...
router = input("Which router you want??? 1. Quickswap 2. SushiSwap\n")

if(router == '2'):
    RouterContractAddress = sushi
else:
    RouterContractAddress = quick
...
usdt_addr = web3.toChecksumAddress("0xc2132d05d31c914a87c6611c10748aeb04b58e8f")

#BUSD address for busd pool
usdc_addr = web3.toChecksumAddress("0x2791bca1f2de4661ed88a30c99a7a9449aa84174")

# WMatic pool
wmatic_addr = web3.toChecksumAddress("0x0d500b1d8e8ef31e21c99d1db9a6444d3adf1270")

#dai pool
dai_addr = web3.toChecksumAddress("0x8f3cf7ad23cd3cadbd9735aff958023239c6a063")

#weth pool
weth_addr = web3.toChecksumAddress("0x7ceb23fd6bc0add59e62ac25578270cff1b9f619")
...
contract = web3.eth.contract(address=RouterContractAddress, abi=panabi)
...
pool = input("which pool does the token have??? \n1. USDTpool \n2. USDCpool \n3. WMATICpool  \n4. daipool  \n5. WETHpool \n")
...
fsymbolcont = web3.eth.contract(spend, abi=sellAbi)
fsymbol = fsymbolcont.functions.symbol().call()

#Approve Token before Selling
start = time.time()

al_value = sellTokenContract.functions.allowance(sender_address ,RouterContractAddress).call()
ap_quan = sellTokenContract.functions.balanceOf(sender_address).call()

if int(al_value) <= int(ap_quan):
    approve = sellTokenContract.functions.approve(RouterContractAddress, 115792089237316195423570985008687907853269984665640564039457584007913129639935).buildTransaction({
            'from': sender_address,
            'chainId': 137,
            'gas': 300000,
            'maxFeePerGas': web3.toWei('300','gwei'),
            'maxPriorityFeePerGas': web3.toWei('80','gwei'),
...
while (zeroOne != '0'):
    ...
    print(f"Swapping {readable} {symbol} for {fsymbol}")

    #Swaping exact Token for ETH 
    amounts = contract.functions.getAmountsOut(balance, [contract_id, spend]).call()

    readable = web3.fromWei(amounts[1],'ether')
    readable = readable - (readable*slippage/100)

    amountOutMin = web3.toWei(readable, 'ether')
    ...
    zeroOne = input("Do you wnat to go again??? 0 to exit")